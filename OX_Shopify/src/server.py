...
# shopify.Session.setup(api_key=os.environ.get('SHOPIFY_API_KEY'), secret=os.environ.get('SHOPIFY_SECRET'))
...
def order_log():
    ...
    global ACCESS_TOKEN
    if ACCESS_TOKEN:
        return render_template('order_log.html', shop=shop)
...
def plans():
    ...
    global ACCESS_TOKEN
    if ACCESS_TOKEN:
        return render_template('plans.html', shop=shop)
...
def support():
    ...
    global ACCESS_TOKEN
    if ACCESS_TOKEN:
        return render_template('support.html', shop=shop)
